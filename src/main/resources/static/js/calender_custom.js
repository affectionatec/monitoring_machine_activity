...
(function ($) {
	...
	$.fn.calendar = function (settings) {
		//Each context has different contents and cannot be placed in default_ Settings,
		...
		function genHtm(context) {
			...
			let disabledDateList = getDisabledDateList();
						...
						if (day <= monthDayNum && (i > 0 || j >= startDay)) {
							...
							if (isChecked(rec.year, rec.month, day, context) >= 0) {
								...
								if (context.blurBefore && (context.blurBefore instanceof Date)
									...
									let disabledFlag = 0;
									if (disabledDateList ? disabledDateList.length : 0) {
										disabledDateList.map((ele, index) => {
											if (ele.split('-')[0] == rec.year + '' && ele.split('-')[1] == rec.month + 1 + '' && ele.split('-')[2] == day + '') {
												disabledFlag = 1;
												htm.push('blur')
											}
										})
									}
									!disabledFlag ? htm.push('day') : '';
								}
							}
...
// calendar_custom component usage:(It supports single selection and multiple selection, and can support to disable some dates)
//
// JS part:
...
// 	var today = new Date();
// 	$.fn.calendar({target:".input_date_multi_blur", mode:'multi', blurBefore:today, startDate:today});
// 	$.fn.calendar({target:".input_date_single_blur", mode:'single', blurBefore:today, startDate:today});
// });
//
// html part
...
// <input type="text" style="width: 300px" class="input_date_single_blur"><br/><br/>
//
// Multi selection calendar with blur<br/>
// <input type="text" style="width: 300px" class="input_date_multi_blur"><br/><br/>